 ...
 * Optimized version of the footer animation for Webflow landing page
 */
function createWebflowGridAnimation() {
    // Find the grid container
    const gridElement = document.querySelector('.grid-container');
    if (!gridElement) {
        console.error("Grid container not found. Make sure you have an element with class 'grid-container'");
        return;
    }
    
    console.log("Starting Webflow grid animation");
    ...
    function setupResizeHandler() {
        ...
        const handleResize = () => {
            ...
            debounceTimeout = setTimeout(() => {
                ...
                // Only reinitialize if the grid size has changed significantly
                if (Math.abs(columns - newColumns) > 2 || Math.abs(rows - newRows) > 2) {
                    // Clear lines array
                    lines.length = 0;
    ...
    // Return the init function to allow controlling when animation starts
    return init();
}
